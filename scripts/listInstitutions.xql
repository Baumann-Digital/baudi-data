...
let $objects := for $object in $coll
                 	let $documentUri := document-uri($object/root())
                 	let $doc := doc($documentUri)
                 	let $fileID := $doc/node()/@xml:id
                    let $objName := $doc//tei:orgName
                 	order by $objName[1] => replace('» ','') => replace('»','')
                 	return
                 	   (:<org xml:id="{$fileID}" xmlns="http://www.tei-c.org/ns/1.0">{$objName}</org>:)
                 	   $object
return
    <listOrg n="{count($coll)}" xmlns="http://www.tei-c.org/ns/1.0">
        {$objects}
    </listOrg>